/* ===== Deer QA WebApp v1.8.5 =====
 * ✅ 修正 shareSite not defined
 * ✅ 全功能整合版：AI麋鹿 / 頁面搜尋 / 導覽 / 更新提示
 * ✅ 安全事件綁定，防止 null.addEventListener
...
function init(){
  ...
  initStats();render();firstTour();
  showChangelogIfNew("1.8.5");
}
...
function render(){
  ...
  $list.querySelectorAll('.q').forEach(btn=>{
    ...
    btn.addEventListener('keydown',e=>{if(['Enter',' '].includes(e.key)){e.preventDefault();toggleItem(btn.closest('.item'));}});
  });
  $list.querySelectorAll('[data-copy]').forEach(el=>{
    el.addEventListener('click',()=>{const qa=getItem(el);navigator.clipboard.writeText(`Q: ${qa.q}\nA: ${qa.a}`);toast('已複製 Q&A');});
  });
}
...
function openAI(){
  ...
  if(!p.dataset.boot){pushMsg('assistant','嗨～輸入你的問題，我會在資料庫中找答案！');p.dataset.boot='1';}
}
...
async function onAsk(e){
  ...
  const typing=pushTyping();const ans=await localAnswer(text);typing.remove();
  const msg=pushMsg('assistant',ans.rendered);
  msg.dataset.sourceQ=ans.sourceQ||'';msg.dataset.conf=ans.confidence.toString();
...
function buildIDF(){
  ...
  const df=new Map();docs.forEach(d=>{const terms=new Set(tokenize(d));terms.forEach(t=>df.set(t,(df.get(t)||0)+1));});
  const N=docs.length;IDF=(term)=>Math.log(1+N/((df.get(term)||0)+0.5));return IDF;
}
async function localAnswer(query){
  ...
  const top=scored[0],second=scored[1]||{score:top.score};
  const topK=scored.slice(0,Math.min(10,scored.length));
  const maxS=Math.max(...topK.map(s=>s.score)),minS=Math.min(...topK.map(s=>s.score));
  ...
  const suggestions=scored.slice(1,4)
    .map(s=>`<button class="btn s-btn" data-sug="${escapeHTML(s.item.q)}">${escapeHTML(s.item.q)}</button>`).join(' ');
  const rendered=`Q：${escapeHTML(top.item.q)}<br>A：${escapeHTML(top.item.a).replace(/\n/g,'<br>')}`+
    (suggestions?`<div class="s-wrap" style="margin-top:8px">${suggestions}</div>`:'');
  return {rendered,sourceQ:top.item.q,confidence:conf};
}

function rank(q){
  const qn=norm(q),idf=buildIDF();
  return window.DEER_QA.map(item=>{
    const q0=norm(removeAliases(item.q)),a0=norm(removeAliases(item.a));
    const doc=q0+' '+a0;
    const sJ=jaccard(qn,doc);
    const vecQ=tfidfVector(tokenize(qn),idf),vecD=tfidfVector(tokenize(doc),idf);
    const sC=cosine(vecQ,vecD),cov=coverage(qn,q0),pos=positionBoost(qn,q0);
    const exact=q0.includes(qn)?0.08:0;
    const score=0.5*sJ+0.3*sC+0.12*cov+0.06*pos+exact;
    return {item,score};
...
function cosine(a,b){let dot=0,na=0,nb=0;for(const[t,wa]of a){const wb=b.get(t)||0;dot+=wa*wb;na+=wa*wa;}for(const[,wb]of b){nb+=wb*wb;}return na&&nb?dot/Math.sqrt(na*nb):0;}
...
function pushMsg(role,content){
  ...
  av.innerHTML=role==='assistant'?`<img alt="AI圖" src="${IMAGES['AI圖']}">`
  ...
  wrap.appendChild(av);wrap.appendChild(body);id('ai-messages')?.appendChild(wrap);return wrap;
}
function pushTyping(){
  const wrap=document.createElement('div');wrap.className='msg typing assistant';
  wrap.innerHTML=`<div class="avatar assistant"><img alt="AI圖" src="${IMAGES['AI圖']}"></div>
  <div class="body"><div class="role">AI麋鹿</div><div class="bubble"><span class="dot"></span><span class="dot"></span><span class="dot"></span></div></div>`;
  id('ai-messages')?.appendChild(wrap);scrollBottom();return wrap;
}
